define([
	...
	var StackContainer = declare("dijit.layout.StackContainer", _LayoutWidget, {
		...
		buildRendering: function(){
			...
			this.stackContainerDiv = domConstruct.place("<div class='containerNodeWrapper'>",	this.domNode)
		},
		...
		_setupChild: function(/*dijit/_WidgetBase*/ child){
			...
			var childNode = child.domNode,
				wrapper = domConstruct.place(
					"<div role='tabpanel' class='" + this.baseClass + "ChildWrapper '>",
					child.domNode,
					"replace"),
			...
			this._addContainerNodeWrapper(wrapper);
		},
		_addContainerNodeWrapper:function(wrapper){
			this.stackContainerDiv.appendChild(wrapper);
			this.stackContainerDiv.style.width =(this.getChildren().length + .5)*100 + "%"; 
			this.containerNode = this.stackContainerDiv;
		...
		_transition: function(newWidget, oldWidget /*===== ,  animate =====*/){
			var childIndex =this.getChildren().indexOf(newWidget);
			var numStr = (newWidget.domNode.parentElement.style.width).split("px")[0]*childIndex;

			this.containerNode.style.webkitTransform = "translate3d(-"+numStr+"px, 0, 0)"
			// summary:
		...
		_setAnimation:function(){
			var activeDomStyleProperties = this.domNode.getClientRects();
			for (var k = 0; k != this.containerNode.children.length; k++) {
				var currentChild = this.containerNode.children[k];

				console.log("currentChild widgth  and height before",currentChild.style.width,"--",currentChild.style.height)
				for (var i = 0; i != activeDomStyleProperties.length; i++) {
				    var activeDomStyleProperty = activeDomStyleProperties[i];
				    console.log("activeDomStyleProperty widgth  and height before",activeDomStyleProperty.width,"--",activeDomStyleProperty.height)
				    currentChild.style.width = ((activeDomStyleProperty.width)) + 'px';
				    currentChild.style.height = ((activeDomStyleProperty.height)) + 'px';
					currentChild.style.float = "left";
					currentChild.style.display = "inline";
				}
				console.log("currentChild widgth  and height after",currentChild.style.width,"--",currentChild.style.height)
				this.containerNode.children[k] = currentChild;
			}
		},
